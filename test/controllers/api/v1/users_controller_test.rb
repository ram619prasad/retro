...
class Api::V1::UsersControllerTest < ActionDispatch::IntegrationTest
  ...
  context '#signin' do
    ...
    should 'return api_token if credentials are correct' do
      post signin_api_v1_users_url, params: {email: @user.email, password: @user.password}
    ...
    should 'return error when invalid email is passed' do
      post signin_api_v1_users_url, params: {email: 'invalid@temp.com', password: @user.password}
    ...
    should 'return error when invalid password is passed' do
      post signin_api_v1_users_url, params: {email: @user.email, password: 'invalidPWD'}
  ...
  context '#signup' do
    setup do
      @params = { email: 'ram@gmail.com', name: 'Ram Prasad', password: 'BigP@ssW0rd'}
    end
  ...
  context '#show' do
    ...
    should 'return the user json_response when Authorization header is passed' do
      get api_v1_user_url(@user.id), headers: {'Authorization' => api_token}

      assert_response :success
      assert json_response.has_key?('data')

      [:id, :type, :attributes, :links].each do |attr|
        assert json_response['data'].has_key?(attr.to_s), "#{attr}.to_s is not present in data key of response"
      end
      assert_equal json_response['data']['id'].to_i, @user.id
      assert_equal 'users', json_response['data']['type']
      assert_equal "/api/v1/users/#{@user.id}/profile", json_response['data']['links']['self']


      [:name, :email, :title, :boards].each do |attr|
        assert json_response['data']['attributes'].has_key?(attr.to_s), "#{attr}.to_s is not present in attributes key of response"
      end
      assert_equal json_response['data']['attributes']['name'], @user.name
      assert_equal json_response['data']['attributes']['email'], @user.email
      assert_equal json_response['data']['attributes']['title'], @user.title
      assert_equal json_response['data']['attributes']['boards'], @user.boards.count
    end
  end
